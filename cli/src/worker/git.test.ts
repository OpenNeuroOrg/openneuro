...
Deno.test("adds git and annexed content given a directory of files", async (t) => {
...
LICENSE annex.largefiles=nothing`),
  ...
  await t.step("git worker ready", async () => {
  ...
  await t.step("run git worker commands", async () => {
    // Configure worker
    worker.postMessage({
      "command": "setup",
  ...
  await t.step("check repo contents", async () => {
    const expectedFiles = [
