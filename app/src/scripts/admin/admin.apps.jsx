      ...
      .map((app, index) => {
        ...
        return (
          <Panel
            ...
            eventKey={index}
            key={index}
            className="col-xs-12 job-panel-wrap">
            <div className="job-panel-header clearfix col-xs-12">
              <div className="row">
                <div className="col-xs-4 job-col">
                  <label>{app[appName][0].jobDefinitionName} Version</label>
                </div>
    ...
    let list = apps.reverse().map((app, index) => {
      ...
      let activeStatus = (
        <span className="label label-success ">
          <i className="fa fa-check-circle" aria-hidden="true" /> {app.status}
        </span>
      )
      let deletedStatus = (
        <span className="label label-warning">
          <i className="fa fa-exclamation-triangle" /> DELETED
        </span>
      )
          ...
          <div className="row">
            ...
            <div className="col-xs-3 job-col">
              <div className={app.status}>
                {app.deleted ? deletedStatus : activeStatus}
              </div>
            </div>
            <div className="col-xs-2  job-col last">
              <button
                className="tool cte-edit-button btn btn-admin fade-in"
                onClick={this._editJobDefinition.bind(this, app)}>
                <i className="fa fa-pencil" /> Edit{' '}
              </button>
              <WarnButton
                action={this._deleteJobDefinition.bind(this, app)}
                icon="fa-trash"
                message="Delete"
                lock={app.deleted}
              />
