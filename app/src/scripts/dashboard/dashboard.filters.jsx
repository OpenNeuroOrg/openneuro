...
export default class Filters extends React.Component {
  ...
  render() {
    ...
    let incompleteFilter = this._button(
      'incomplete',
      'fa-warning',
      'Incomplete',
    )
    let sharedFilter = this._button('shared', 'fa-user', 'Shared with me')
    let invalidFilter = this._button(
      'invalid',
      ...
      filterButtons = (
        <div className="filters">
          ...
          <button
            className={
              filters.length === 0
                ? 'active btn-filter filter-all'
                : 'btn-filter filter-all'
            }
            onClick={this._filter.bind(this, 'reset')}>
            All
          </button>
          {incompleteFilter}
          {sharedFilter}
    ...
    return <span>{filterButtons}</span>
  }
      ...
      <button
        className={
          filters.indexOf(filter) > -1
            ? 'active btn-filter filter-' + filter
            : 'btn-filter filter-' + filter
        }
        onClick={this._filter.bind(this, filter)}>
        <i className={'fa ' + icon} />
