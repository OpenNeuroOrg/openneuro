...
export default class Validation extends React.Component {
  ...
  _header(errors, warnings) {
    ...
    if (errors.length > 0) {
      errs = (
        <span className="label text-danger pull-right">
          {' '}
          {errors != 'Invalid'
      ...
      if (warnings.length > 0) {
        warnMessage = (
          <span className="message error fade-in">
            ...
            in your dataset. You are not required to fix warnings, but doing so
            will make your dataset more BIDS compliant.
          </span>
        )
      }
    }
  ...
  _togglePanel() {
    if (this.props.errors.length === 0 && this.props.warnings.length === 0) {
      return
    }
