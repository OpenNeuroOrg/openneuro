...
const successful_run = {
    ...
    analysis: {
        ...
        logstreams: [
            {
                name:
                    'bids-example-nell/default/5c35f0d9-1ff1-49fa-b7a7-0175a53ef4ca',
                environment: [
            ...
            {
                name:
                    'bids-example-nell/default/eb40a865-750c-4f91-b71e-c4b9ed90ccae',
                environment: [
                    { name: 'BIDS_OUTPUT_BUCKET', value: 'openneuro.outputs' },
                    {
                        name: 'BIDS_DATASET_BUCKET',
...
describe('dataset/dataset.job.run/JobAccordion', () => {
    it('renders a successful run', () => {
        const wrapper = shallow(
            <JobAccordion run={successful_run} toggleFolder={props.toggleFolder} displayFile={props.displayFile} />
        );
        expect(wrapper).toMatchSnapshot();
    });
    it('renders a failed run', () => {
        const wrapper = shallow(
            <JobAccordion run={failed_run} />
        );
        expect(wrapper).toMatchSnapshot();
    });
});
